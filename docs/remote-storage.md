...
- The user is optional in the URI, but the effective username is not optional. Resolution is deterministic: use the
  explicit URI user when present; otherwise use the `User` resolved from the user's SSH configuration for the host and
  port; otherwise use `System.getProperty("user.name")`. If the resulting username is empty or unavailable, reject the
  location instead of connecting with an agent-selected identity.
...
- A remote identity is the normalized tuple `(scheme, host, port, effective username, absolute path)`.
- Canonicalization materializes the effective username into the durable URI as `sftp://user@host:port/path`; the
  original omitted-user spelling is not persisted in buffers, recent files, or sessions.
...
| `save` | Require the expected revision for an existing remote document. Acquire the standard SFTP exclusive-create lock, write to a same-directory temporary file, recheck the revision, then use a supported replacement rename while holding the lock. Never truncate the target. |
...
SFTP has no standard compare-and-swap rename. The first backend therefore uses only standard SFTP mechanisms and makes
the conflict guarantee explicit:

1. Create a per-target sidecar lock with SFTP `OPEN` using `SSH_FXF_CREAT | SSH_FXF_EXCL` (for example,
   `document.md.serenity-lock`). The lock contains a random nonce, the effective username, and an expiry timestamp; it
   contains no credential. An existing lock returns a conflict/locked outcome.
2. While holding that advisory lock, recheck the target revision against the expected revision. A mismatch returns
   `Conflict` before upload.
3. Upload to a unique same-directory temporary file and recheck the target revision again. For an existing target,
   replace it with OpenSSH's `posix-rename@openssh.com` extension, or with `SSH_FXP_RENAME` only when the server
   explicitly documents atomic replacement semantics. For a new target, standard `SSH_FXP_RENAME` is sufficient. Remove
   the lock in a finalizer.

The lock is a Serenity/cooperating-writer guarantee, not a protocol-level mutex. Every Serenity SFTP save must honor
the lock; concurrent Serenity saves therefore cannot pass the final recheck and overwrite one another. A server that
does not support standard exclusive-create semantics cannot provide this guarantee, so Save fails before upload with an
explicit unsupported-safe-save error. A server that cannot provide a supported replacement rename can create new
documents but must reject saves to existing documents; it must never emulate replacement with delete-then-rename. The
first implementation therefore has a concrete compatibility boundary: standard SFTP v3 plus exclusive create for
locking, and either OpenSSH `posix-rename@openssh.com` or a tested/documented equivalent for replacing an existing
target.

An external writer that ignores the advisory lock can still change the target between the final recheck and rename;
ordinary SFTP cannot detect that race atomically. The supported semantics for that case are documented last-writer-wins:
the save reports the rename result, and a later open observes the resulting content. Serenity must not describe this as
strong conflict protection, and it must never silently claim a `Conflict` that the protocol cannot prove. A future
server-side CAS extension would be a separate backend capability decision.

The temporary upload name is unique, is created beside the target, and is removed best-effort after success or failure.
If lock cleanup or rename completion is ambiguous, the buffer remains dirty and the user receives an indeterminate-save
...
1. **Add an SFTP transport adapter.**
   ...
   - Implement SSH-agent authentication, known-host verification, the timeout policy, cancellation, and typed error
     mapping.
   - Resolve and materialize the effective username before opening the connection; an SSH agent must never be treated
     as the source of username identity.
...
2. **Extend the document-storage boundary and implement `SftpDocumentStorageProvider`.**
   ...
   - Enforce UTF-8 and the 50 MiB limit, optimistic conflict detection, standard exclusive-create sidecar locks,
     same-directory temporary uploads, and rename cleanup.
   - Test standard SFTP servers that support and reject `SSH_FXF_CREAT | SSH_FXF_EXCL`, servers with and without
     OpenSSH `posix-rename@openssh.com` (or a documented atomic-replacement equivalent), concurrent cooperating saves,
     target changes before each revision check, lock cleanup, new-target saves, and the documented last-writer-wins
     behavior for a non-cooperating external writer. Do not require a conditional-replace/CAS extension.

...
object TextDocumentSyncKind:
  ...
  /** `capabilities.textDocumentSync` is either the kind code directly, or an object whose `change` field carries it
    ...
    * parses -- this client always sent full-document `didChange` notifications before this capability was read at all,
    * so an unset or unrecognized capability must keep doing that rather than silently going quiet.
    */

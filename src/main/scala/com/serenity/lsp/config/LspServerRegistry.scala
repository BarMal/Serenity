...
object LspServerRegistry:
  ...
  private def applyUserOverride(config: LspServerConfig, languageId: LanguageId, userConfig: LspUserConfig): LspServerConfig =
  ...
  private def isAvailable(command: String): Boolean =
    ...
    pathDirs.exists { dir =>
      suffixes.exists { suffix =>
        Files.isExecutable(Paths.get(dir, command + suffix))
      }
    }

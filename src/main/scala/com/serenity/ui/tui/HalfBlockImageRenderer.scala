...
/** Converts a decoded image into a grid of half-block terminal cells: each cell is an upper-half-block glyph (`▀`)
  ...
  * effective vertical resolution -- the technique notcurses, ratatui-image, and Jexer all use for pixel art in a plain
  * ANSI-color terminal (out of scope here: Sixel/Kitty/iTerm2 true-pixel protocols, which need a different capability
  * entirely).
  *
...
object HalfBlockImageRenderer:
  ...
  /** Nearest-neighbor downsamples `image` to `cellsWide` columns and `cellsHigh * 2` sub-pixel rows, then pairs each
    ...
    * comes back as `None`, telling the caller to leave whatever was already on screen there instead of painting over it
    * with an arbitrary color; a cell with exactly one transparent half substitutes `fallback` for that half only.
    */
  def render(
    image: BufferedImage,
    cellsWide: Int,

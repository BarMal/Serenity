...
object ConfigurableThemeManager:
  ...
  private def convertSyntaxColors(syntax: SyntaxColors, defaultBackground: TextColor): Either[String, Map[SyntaxElement, ThemeColor]] =
    ...
    val results = conversions.map { case (element, config) =>
  ...
  private def convertSyntaxElementConfig(config: SyntaxElementConfig, defaultBackground: TextColor): Either[String, ThemeColor] =
    for
      foreground <- ColorParser.parseColor(config.foreground)
      background <- if config.background == "default" then Right(defaultBackground) 
...
class ConfigurableThemeManager(loader: ThemeConfigLoader):
    ...
    com.serenity.ui.theme.DefaultThemes.allInternal.get(themeName) match
      ...
      case None =>
        ...
        loadThemeFromFile(userFile)
          ...
          .handleErrorWith(error => IO.raiseError(
  ...
  def listAvailableThemes: IO[List[String]] =
    for
      userThemes <- loader.listAvailableThemes(loader.getUserThemesDirectory)

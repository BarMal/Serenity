...
object AccessibilitySnapshot:
  ...
  private def surfaceControls(state: AppState, scene: UiSceneSnapshot): List[AccessibleNode] =
    scene.nodesInPaintOrder.flatMap {
      case node @ SceneNode(SceneNodeId.Surface(surfaceId), _, _, _, _, _) =>
        state.surfaceById(surfaceId).toList.flatMap(surface => controlsFor(surface, node, state))
      case _ => Nil
    }

  private def controlsFor(surface: UiSurface, node: SceneNode, state: AppState): List[AccessibleNode] =
        ...
        page.launchActions.zipWithIndex.map {
          case (action, index) =>
            AccessibleNode(
              ...
              actionBounds(node.contentRect, index, page.launchActions.size)
            )
        }
      case SurfaceContent.CommandPalette(runner) => commandControls(surface.id, runner, node.contentRect, state)
      case SurfaceContent.CommandPaletteSubmenu(runner, _, _) =>
        commandControls(surface.id, runner, node.contentRect, state)
      case _ => Nil
  ...
  private def commandControls(
    ...
    runner.visibleItems.zipWithIndex.map {
      case (item, index) =>
        ...
        AccessibleNode(
          ...
          actionBounds(bounds, index, runner.visibleItems.size)
        )
    }

...
object LayoutEngine:
  ...
  def calculateLayoutWithUI(
    ...
    val uiElementGap   = FloatingSurfaceGeometry.reservedCellRows(state.config.uiElementGap)
    ...
    baseLayout.copy(
      ...
      floatingSurfacePlacements = floatingSurfacePlacements,
      ...
      state.config.commandRunnerVisibleRows
        .map(rows =>
          SurfaceFrameLayout.frameHeightForItemRows(
            ...
            itemGapRows = FloatingSurfaceGeometry.reservedCellRows(state.config.commandRunnerItemGapRows)
          )
        )
    ...
    anchorFrameOpt match
      ...
      case Some(anchorFrame) =>
        val gapRows = surfaces.headOption
          .map(surface => FloatingSurfaceGeometry.reservedCellRows(floatingCursorGapRows(state, surface.content)))
          .getOrElse(0)
        val stackGapRows    = FloatingSurfaceGeometry.reservedCellRows(floatingStackGapRows(state))
        val availableBottom = anchorFrame.contentRect.bottom

...
object LayoutEngine:
  ...
  def calculateLayoutWithUI(
    ...
    val uiElementGap   = FloatingSurfaceGeometry.requiredCellRows(state.config.uiElementGap)
  ...
  private def floatingStackGapRows(state: AppState): Int =
    math.max(
      ...
      FloatingSurfaceGeometry.requiredCellRows(state.config.uiElementGap)
    )
      ...
      state.config.commandRunnerVisibleRows
        .map(rows =>
          SurfaceFrameLayout.frameHeightForItemRows(
            ...
            itemGapRows = FloatingSurfaceGeometry.requiredCellRows(state.config.commandRunnerItemGapRows)
          )
        )

...
private[layout] object CommandPaletteContentResolver:
  ...
  /** Renders a `GroupItem` row -- shared by the palette (whose label is a breadcrumb once a search is active) and the
    * settings surface (whose label is always the plain group name).
    */

...
object TabBarSurfaceComposition:
  ...
  private def layOut(entries: List[TabListEntry], activeBufferId: Option[BufferId], rect: LayoutRect): TabBarLayout =
    val window = visibleWindow(entries, activeBufferId, rect.width)
    val tabsWidth =
      if window.hiddenCount > 0 then math.max(0, rect.width - OverflowIndicatorWidth - GapColumns) else rect.width

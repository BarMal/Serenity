...
/** Declarative composition plan for the command runner (issue #819, slice 2: command runner and submenu) -- the
  * palette and the settings surface it also hosts (issue #1059), resolved into one paint/hit-test/focus plan the
  * same way `ModalSurfaceComposition` and `ContextMenuSurfaceComposition` already do for the other migrated
  * surfaces.
  *
  ...
  * `CommandPaletteContentResolver` itself uses for painting. That parity matters: the window's absolute item index
  * is computed once per row here, at the same place its position is assigned, so a row after another item's
  ...
  *
  * Scope note: this composition's `hitAt` covers only the cell/keyboard-coordinate hit-test path. The command
  * runner's sub-cell fractional-pixel hover/click path (`FloatingSurfaceGeometry`, exercised by
  * `CommandRunnerMouseSpec`'s "fractional floating pixel offset" tests) is a distinct, already-tested feature this
  * migration does not touch -- callers keep using `MouseHitTestGeometry.overlayItemIndex` for that case.
  */
object CommandRunnerSurfaceComposition:
  ...
  private def forPalette(
    ...
    assemble(content, entries, Some(header), footer.map(OverlayRow(_)), keyHint.map(OverlayRow(_)), itemGapRows, itemTargetRows, hasFooter, hasKeyHint)
  ...
  private def forSettingsSurface(
    ...
    val header         = CommandPaletteContentResolver.breadcrumbHeader(runner.settingsSurfaceBreadcrumbLabels, Option.when(searchTerm.nonEmpty)(searchTerm))
    ...
    assemble(content, entries, Some(header), footer.map(OverlayRow(_)), keyHint.map(OverlayRow(_)), itemGapRows, itemTargetRows, hasFooter = true, showKeyHints)
  ...
  private def paletteRow(runner: CommandRunner, item: CommandSurfaceItem, selected: Boolean): OverlayRow =
    item match
      case CommandSurfaceItem.CommandItem(command) =>
        val prefix =
          if runner.searchTerm.isEmpty then "" else s"[${CommandPaletteContentResolver.categoryLabel(command.category)}] "
  ...
  private def assemble(
    ...
    val headerBoxes = header.toList.flatMap(row =>
      slots.collectFirst { case SurfaceContentRowSlot(SurfaceContentRowKind.Header, y) => toBox(row, rowRect(bounds, y - content.y), None) }
    )
    ...
    val keyHintBoxes = keyHint.toList.flatMap(row =>
      slots.collectFirst { case SurfaceContentRowSlot(SurfaceContentRowKind.KeyHint, y) => toBox(row, rowRect(bounds, y - content.y), None) }
    )
    val footerBoxes = footer.toList.flatMap(row =>
      slots.collectFirst { case SurfaceContentRowSlot(SurfaceContentRowKind.Footer, y) => toBox(row, rowRect(bounds, y - content.y), None) }
    )

...
/** East-Asian-Width classification for the codepoints that matter to a monospace terminal grid, backed by ICU4J's
  ...
  * matches what most terminal emulators default to (#1277), and is the same choice the old hand-rolled table made
  * for the codepoints it covered at all. Unlike that table, this also gets the wide singletons that sit outside the
  * old table's blocks right (U+231A, U+1F004, U+1F0CF and friends), since the property covers every codepoint rather
  * than a fixed list of ranges.
  *

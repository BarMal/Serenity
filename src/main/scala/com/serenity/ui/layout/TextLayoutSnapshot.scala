...
object TextLayoutSnapshot:
  ...
  /** Column-based document layout (issue #1338, Phase 1): the same wrapped visual-line stream `fromBuffer` slices into
    * one screenful, partitioned instead into `columnCount` groups of `buffer.viewport.visibleLines` rows each -- one
    * per column -- starting from the viewport's own `topLine`/`topVisualLine` (already snapped to a column boundary by
    * `CursorViewport.adjustForCursorColumnMode`). Reuses the same wrap logic (`collectVisualLines`) unchanged; a
    * document that runs out of lines before filling every column simply yields a shorter final chunk.
    */

...
object CommandRunnerRenderer:
  ...
  private def drawSearchInput(
    ...
    val displayText = if searchText.length <= width - prompt.length then
      prompt + searchText
    else
      prompt + searchText.takeRight(width - prompt.length)
  ...
  private def drawCommandItem(
    ...
    val truncatedName = if command.name.length <= nameWidth then
      command.name.padTo(nameWidth, ' ')
    else
      command.name.take(nameWidth - 1) + "…"
    
    val truncatedDesc = if command.description.length <= descWidth then
      command.description
    else
      command.description.take(descWidth - 1) + "…"

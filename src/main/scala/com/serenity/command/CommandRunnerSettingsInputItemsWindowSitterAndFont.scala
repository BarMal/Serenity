...
/** Window-sitter behavior and font-size input items. Split out of `CommandRunnerSettingsInputItems.build` to keep
...
private[command] object CommandRunnerSettingsInputItemsWindowSitterAndFont:
  ...
  CommandSurfaceItem.InputItem(
    ...
    isDecimal = false,
    parse = text =>
      CommandRunnerSettingsInputItems.nonEmptyCommaList(text).map(values =>

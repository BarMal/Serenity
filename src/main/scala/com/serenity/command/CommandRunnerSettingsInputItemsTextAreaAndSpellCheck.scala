...
/** Editor text-area margin and spell-check input items. Split out of `CommandRunnerSettingsInputItems.build` to
...
private[command] object CommandRunnerSettingsInputItemsTextAreaAndSpellCheck:
  ...
  CommandSurfaceItem.InputItem(
    ...
    isDecimal = false,
    parse = text =>
      CommandRunnerSettingsInputItems.nonEmptyCommaList(text).map(commandIntentArg =>
    ...
    parse = text =>
      Some(
        CommandIntent.Settings(
          SettingsIntent.SpellCheck(SpellCheckIntent.SetSpellCheckDictionaryPaths(CommandRunnerSettingsInputItems.commaListPreserveCase(text)))
        )
      ),
    category = CommandCategory.Settings,
    acceptsFreeText = true
  ...
  CommandSurfaceItem.InputItem(
    ...
    parse = text =>
      Some(
        CommandIntent
          .Settings(SettingsIntent.SpellCheck(SpellCheckIntent.SetSpellCheckWords(CommandRunnerSettingsInputItems.commaList(text))))
      ),
    category = CommandCategory.Settings,
    acceptsFreeText = true
  )

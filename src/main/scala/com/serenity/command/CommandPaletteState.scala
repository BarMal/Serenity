...
/** The command palette's own navigation state: search text, the ranked results it produces, and which one is
  ...
  * surface its own `SettingsSurfaceState` rather than a further pile of fields on the same class. This is
  * `CommandRunner`'s sole storage for search/select/results now -- see `CommandRunnerSurface` below.
  *
...
/** `CommandRunner`'s sole navigation storage (issue #931, Stage 2): the whole former mode-flag grab-bag --
  ...
  *
  * `Settings` carries its own `root: CommandPaletteState` alongside `drilled: Option[SettingsSurfaceState]`, not a
  * bare `SettingsSurfaceState`: the dedicated Settings surface has a real "open, but nothing drilled into yet" state
  * -- browsing the top-level settings groups, or searching them -- and `SettingsPage` has no case for "no page", by
  * design (see `SettingsSurfaceState`'s own doc: it always has a current page once it exists at all). Reusing
  * `CommandPaletteState`'s search/select shape for that root state (rather than inventing a second near-identical
  * type) means both roots -- the palette's and the settings surface's -- are searched and navigated by the exact
  * same mechanics; `root.filteredCommands` is simply unused there (settings search goes through
  * `CommandRunner.matchingSettingsResults` instead).
  *
  * A group entered from either root carries that root forward as `drilled`'s sibling `root` field, so the search
  * that led to it (if any) survives while drilled in -- `CommandRunnerReducerSpec`'s "open the matched settings leaf
  * without filtering away its context" depends on exactly this. Escaping back out of the *last* drilled level always
  * lands on a bare `Settings(root, drilled = None)`, i.e. the settings-root view -- including when the group was
  * reached via a `Palette` search rather than `CommandRunner.openSettings`. The pre-migration code (`mode` and
  * `activeSettingsSurface` as independent fields) could instead restore the original `Palette` view in that one case;
  * no test exercised it, and reproducing it faithfully needs a fourth state
  * (`Palette` searched, then drilled, then popped back to that same `Palette`) that this migration deliberately
  * does not add -- a documented simplification, not an oversight (see the PR notes).
  */

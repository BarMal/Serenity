...
private[command] object CommandRunnerSearch:
  ...
  /** issue #1048: fuzzy score of `term` against one target string, or `None` when it doesn't match at all -- the
    ...
    * whitespace-separated token, all of which must match somewhere in `target`) and the better of the two wins --
    * this is what lets a reordered or scattered query like "numbers line" still match "Toggle Line Numbers" even
    * though it is not, as a whole string, an in-order subsequence of the target. This restores the old per-token AND
    * matcher's coverage for multi-word queries without giving up the single whole-string match's scoring.
  ...
  /** Tiers 1-4 of `fuzzyScore` against a single, already-normalized needle (either the whole query, or one token of
    * it in the AND fallback below).
    */
  ...
  /** issue #1048 regression fix: the whole-query sequence match above requires every character of `needle` to
    * appear in `target` in query order, so a reordered or scattered multi-word query (words present, but not in the
    * order typed) fails it entirely -- the old per-token AND matcher this replaced did not have that limitation.
    * Splits `needle` on whitespace and requires every token to independently match somewhere in `haystack` (AND
    * semantics); `None` if there is only one token (no benefit over the whole-query match) or if any token fails to
    * match at all. The combined score is the mean of the per-token scores, so it can only win over the whole-query
    * score when the latter is `None` or comes from the weaker scattered-subsequence tier.
    */

...
/** Distinguishes, for the *configured* command-runner cursor-peek modifier specifically, a held bare press (->
  * "peek") from a double-tap (-> "open fully") from neither -- by composing [[ModifierTapDetector]]'s existing
  * press/release/window state machine rather than re-implementing its timing logic (the window is
  * [[ModifierTapDetector.WindowMillis]], matched by `SurfaceConfig.commandRunnerCursorPeekTapWindowMillis`'s default
  * of 200L).
  *
  * ===Optimistic peek, upgraded to a double-tap===
  *
  * The first bare press of the configured modifier always begins a peek immediately ([[Outcome.PeekBegin]]) --
  * there is no way to know yet whether it is the start of a hold or the first half of a double-tap, so the caller
  * shows the peek optimistically. If a second press of the same modifier follows within the window (with an
  * intervening release, per `ModifierTapDetector`'s `bf5de9e0` guard against mistaking held-key auto-repeat for a
  * second tap), that peek is upgraded to a full open ([[Outcome.DoubleTapOpen]]) instead. Releasing the modifier
  * while a peek is showing signals the caller to begin the settle-out ([[Outcome.PeekEnd]]) -- for a plain hold this
  * is the end of the gesture; for the first half of a double-tap it is superseded moments later by
  * [[Outcome.DoubleTapOpen]] if the second press arrives in time, which the caller (reducer/effect-handler wiring,
  * not this pure detector) is responsible for reconciling.
  *
  ...
  * releasing any other modifier (including ones with their own pending `ctrl+ctrl`-style tap in
  * `SwingInputHandler`'s separate tracking) is a complete no-op here, leaving any in-progress peek tracking
  * untouched. A non-modifier key press still cancels a pending peek, matching
...
object CursorPeekDetector:
  ...
  enum Outcome:
    ...
    /** The second press of the configured modifier within the window, following an intervening release: abandon
      * any in-progress peek and open the command runner fully, exactly like today's normal open.
      */
    case DoubleTapOpen(state: CursorPeekState)

    /** A release that ended an in-progress peek (or a non-modifier key press that cancelled a pending one): begin
      * the settle-out presentation.
      */

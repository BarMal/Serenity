    ...
    def parse(config: AppConfig, key: String, value: String): Option[AppConfig] =
      ...
      if densityKeys.contains(key) then InterfaceDensity.fromConfigKey(trimmed).map(config.withInterfaceDensity)
        ...
        trimmed.toDoubleOption
          .filter(gap => gap.isFinite && gap >= AppConfig.MinUiElementGap && gap <= AppConfig.MaxUiElementGap)
  ...
  object Schema:
    ...
    private def parseCommandRunnerItemGapRows(value: String): Option[Double] =
      value.toDoubleOption.filter(rows =>
        rows.isFinite && rows >= AppConfig.MinCommandRunnerItemGapRows && rows <= AppConfig.MaxCommandRunnerItemGapRows
      )
      ...
      value.toLowerCase match
        ...
        case other =>
          other.toDoubleOption
            .filter(rows =>
              rows.isFinite && rows >= AppConfig.MinCommandRunnerCursorGapRows && rows <= AppConfig.MaxCommandRunnerCursorGapRows
            )
            .map(rows => Some(rows))

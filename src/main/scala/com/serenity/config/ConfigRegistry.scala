...
/** Every setting the config file persists, declared once.
  ...
  *
  * "Declared once" is why this file runs long: the list is one flat declaration whose order is load-bearing (see
  * `readOrder`), and cutting it in half only to reassemble it with `++` puts the halves' `field`/`named` constructors
  * out of file scope for no gain. The file-length ratchet carries a baseline entry for it instead.
  */

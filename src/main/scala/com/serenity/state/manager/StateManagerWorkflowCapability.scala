...
final private[manager] class StateManagerWorkflowCapability(
  ...
  /** Opens the directory an Open dialog is targeting as a project root (issue #1525): resolves and validates the
    * target through `fileWorkflow` (reporting back into the dialog on failure, exactly like a regular Open submit),
    * then -- on a confirmed directory -- dismisses the dialog and hands the path to `openProjectRoot`, which the
    * caller supplies so this capability doesn't need its own route to the panel-pinning machinery (`StateManagerPanelEffects`,
    * owned by `StateManagerEffectHandlers`, isn't available to this capability -- issue #1525 reuses it rather than
    * duplicating it).
    */

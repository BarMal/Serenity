...
/** An in-progress tab-bar drag-to-reorder gesture (issue #1079): the tab a primary-button press picked up, tracked in
  ...
  * app can never observe, and the next `MousePress` -- the only reliable "the previous gesture ended" signal
  * available -- always resets this to whatever that press starts (or clears it, if the press missed the tab bar).
  */

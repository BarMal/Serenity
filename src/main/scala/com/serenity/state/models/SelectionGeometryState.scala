...
/** One visual line's mid-flight highlight extent. `removing` marks a line that is shrinking to a zero-width sliver on
  * its way out (a truncated line, or every line of a selection being cleared entirely) --
  * [[SelectionGeometryState .advance]] drops it once its tween completes, rather than leaving a zero-width entry to
  * paint nothing forever.
  */
...
/** Selection grow/settle (issue #1085 phase 3): mid-flight geometry for one cursor's selection highlight, one
  ...
  * and unused -- not a screen-pixel rect the way `PanelGeometryState`'s is; a selection highlight's real screen
  * position still comes from the renderer's own per-visual-line pixel measurement (`RendererHighlights`), exactly as it
  * does for the live (non-animated) case. This is also why one shared model serves both the GUI's sub-pixel-measured
  * painting and TUI's whole-cell painting: both already resolve a `(startColumn, endColumn)` pair down to pixels/cells
...
final case class SelectionGeometryState(lines: List[SelectionLineGeometry]):
  ...
  def rectFor(bufferLine: Int, startColumn: Int): Option[LayoutRect] =
    lines
      .find(line => line.key.bufferLine == bufferLine && line.key.startColumn == startColumn)
      .map(_.tween.currentValue)

  /** Advances every line's tween by one step, dropping a `removing` line the moment its shrink completes. A
    * non-removing line whose tween completes is kept -- its `currentValue` has already settled at its final rect, which
    * is exactly what the live (non-animated) selection would paint there anyway, so there is nothing wrong with leaving
    * it in place until the whole state is replaced or cleared.
    */
...
object SelectionGeometryState:
  ...
  /** Turns `before`/`after` per-line rects (each a map from a visual line's [[SelectionLineKey]] to its column extent,
    * computed by `state.manager.SelectionGeometry` against the buffer's own text layout) into the tween list this state
    * wraps -- the one place every transition anchor-stability permits is handled uniformly:
    *
    ...
    *   - a key in both, with an unchanged rect, needs no tween at all -- only the line whose extent actually moved (the
    *     one adjacent to the moving focus) is worth animating
    ...
    * whichever keys still have a live (non-complete) tween in `existing` when this is called again before the previous
    * animation finished.
    *

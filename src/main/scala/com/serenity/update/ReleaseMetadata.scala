...
case class ReleaseMetadata(tag: String, assets: List[ReleaseAsset]):
...
object ReleaseMetadata:
  ...
  private case class GitHubRelease(tag_name: String, assets: List[GitHubAsset])

  private given Decoder[GitHubAsset]   = Decoder.forProduct2("name", "browser_download_url")(GitHubAsset.apply)
  private given Decoder[GitHubRelease] = Decoder.forProduct2("tag_name", "assets")(GitHubRelease.apply)

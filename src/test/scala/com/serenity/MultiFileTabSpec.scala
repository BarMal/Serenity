...
class MultiFileTabSpec extends AnyFlatSpec with Matchers:
  ...
  it should "open multiple files in different tabs" in new MultiFileFixture:
    ...
    val buffer1 = stateManager.createBuffer("Content of file 1", Some(java.nio.file.Paths.get("file1.txt"))).unsafeRunSync()
    val buffer2 = stateManager.createBuffer("Content of file 2", Some(java.nio.file.Paths.get("file2.txt"))).unsafeRunSync()
    val buffer3 = stateManager.createBuffer("Content of file 3", Some(java.nio.file.Paths.get("file3.txt"))).unsafeRunSync()
  ...
  it should "handle closing tab with unsaved changes" in new MultiFileFixture:
    ...
    val bufferId = stateManager.createBuffer("Original content", Some(java.nio.file.Paths.get("/tmp/test.txt"))).unsafeRunSync()
  ...
  it should "maintain tab order when adding and removing tabs" in new MultiFileFixture:
    ...
    val buffer1 = stateManager.createBuffer("First tab content", Some(java.nio.file.Paths.get("file1.txt"))).unsafeRunSync()
    val buffer2 = stateManager.createBuffer("Second tab content", Some(java.nio.file.Paths.get("file2.txt"))).unsafeRunSync()
    val buffer3 = stateManager.createBuffer("Third tab content", Some(java.nio.file.Paths.get("file3.txt"))).unsafeRunSync()
    val buffer4 = stateManager.createBuffer("Fourth tab content", Some(java.nio.file.Paths.get("file4.txt"))).unsafeRunSync()
  ...
  it should "handle pane resizing with minimum width constraints" in new MultiFileFixture:
    ...
    var currentBufferId = focusedBufferId
    var visitedBuffers = Set(currentBufferId)
    
    // Navigate through all buffers to ensure they're accessible
    (1 until allBufferIds.size).foreach { _ =>
      stateManager.applyEvent(NextTab).unsafeRunSync()
      val state = stateManager.getCurrentState.unsafeRunSync()
      currentBufferId = state.focusedBufferId.get
      visitedBuffers += currentBufferId
    }
  ...
  it should "preserve tab state across sessions (simulated)" in new MultiFileFixture:
    ...
    val buffer1 = stateManager.createBuffer("File 1 content", Some(java.nio.file.Paths.get("/tmp/file1.txt"))).unsafeRunSync()
    val buffer2 = stateManager.createBuffer("File 2 content", Some(java.nio.file.Paths.get("/tmp/file2.txt"))).unsafeRunSync()
    val buffer3 = stateManager.createBuffer("Untitled buffer", None).unsafeRunSync()
    
    val state = stateManager.getCurrentState.unsafeRunSync()
    
    // When: Simulate session state capture
    val sessionData = state.buffers.values.map { buffer =>
      (buffer.filePath, buffer.content.collect(), buffer.isDirty)
    }.toList
    ...
    sessionData.foreach { case (_, _, isDirty) =>

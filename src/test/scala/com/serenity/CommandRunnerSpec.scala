...
import com.serenity.command.{Command, CommandRegistry, CommandSearcher, CommandRunner}
...
class CommandRunnerSpec extends AnyFlatSpec with Matchers:
  ...
  it should "handle enter to execute selected command" in {
    ...
    val testCommand = Command("test", "Test command", state => {
      executed += "executed"
      IO.unit
    })
    ...
    val registry = CommandRegistry.default
    given CommandRegistry = registry
    val runner = CommandRunner.withCommands(List(testCommand)).activate(registry)

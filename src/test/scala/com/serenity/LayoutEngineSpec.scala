...
class LayoutEngineSpec extends AnyFlatSpec with Matchers:
  ...
  it should "reserve no tab bar strip with a single buffer open" in {
    ...
    // Matches the pre-existing baseline for this exact state/viewport ("use the full workspace width..." above):
    // `AppConfig.default`'s pinned status line already reserves its own bottom gutter row regardless of the tab bar,
    // and line numbers are on by default -- neither of which this test is about, so the reservation under test is
    // "no *additional* row for the tab bar", not "no chrome at all".

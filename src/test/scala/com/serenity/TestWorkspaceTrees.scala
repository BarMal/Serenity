...
/** Builds a simple left-to-right nested horizontal split tree from an explicit pane-id order, for test fixtures that
  * need a valid `Layout.workspaceTree` covering a specific set of panes but don't care about split topology themselves.
  * `Layout.workspaceTree` is now always populated whenever any editor pane exists (#821: the old flat

...
class OverlayViewModelSpec extends AnyFlatSpec with Matchers:
  ...
  it should "preserve fractional pixel gaps for palette, submenu, context menu, and toolbar at DPI-scaled metrics" in {
    ...
    surfaces.zipWithIndex.foreach { (content, index) =>
      val state = stateWithQuickInfo("geometry").copy(
        config = AppConfig.default.withCommandRunnerItemGapRows(0.25).withUiElementGap(0.25),
        uiSurfaces = List(
          UiSurface(
            SurfaceId(s"fractional-$index"),
            content,
            SurfacePresentation.Floating(Some(CursorPosition(1, 2)), SurfacePlacement.BelowCursor)
          )
        ),
      ...
      metrics.foreach { metric =>
        ...
        geometry.items.sliding(2).foreach {
          case Vector(first, second) => second.y - first.y shouldBe metric.lineHeight * 1.25
          case _                     => ()
        }
      }
    }

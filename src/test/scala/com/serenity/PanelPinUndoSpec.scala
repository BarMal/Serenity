...
/** #1016 PR4's acceptance criterion: pinning a panel is undoable, regardless of which of the two production pin/unpin
  ...
  * command runner) and `PanelStateReducer` via `StateManagerSurfaceCapability` (reached directly via `StateManager`'s
  * `pinPanel`/`unpinPanel`, the plain-method replacements #1724 gave the deleted `PanelManager` record, for
  * drag-to-pin/peek-to-pin -- no `ViewIntent` covers pinning arbitrary content or unpinning by surface id). See
  * `HistoryEntrySpec` for `HistoryEntry.PanelChange.restore` in isolation.
  */
class PanelPinUndoSpec extends AnyFlatSpec with Matchers:
  ...
  behavior of "Undoing a panel pin/unpin via StateManager's direct pin/unpin methods (drag-to-pin)"

  it should "restore the previous (unpinned) state after undoing a pin" in new PanelFixture:
    sm.pinPanel(PanelContent.Outline(Nil), PanelPosition.Right, 30).unsafeRunSync()
  ...
  it should "restore the previously pinned surface after undoing an unpin" in new PanelFixture:
    sm.pinPanel(PanelContent.Diagnostics(Nil), PanelPosition.Bottom, 10).unsafeRunSync()
    val afterPin  = sm.getCurrentState.unsafeRunSync()
    val surfaceId = afterPin.pinnedSurfaces.head.id

    sm.unpinPanel(PanelTarget.ById(surfaceId)).unsafeRunSync()
    sm.getCurrentState.unsafeRunSync().pinnedSurfaces shouldBe Nil

...
/** Dedicated unit coverage for `PickerContentResolver` (issue #1421). `SurfaceContentResolverReferencePanelsSpec`
  * covers the theme picker and file search behavior indirectly through `SurfaceContentResolver.resolve`; this adds
  * `resolveThemeCreator` coverage, entirely absent elsewhere, and pins each of the others by directly naming this
  * object. Lives in this package because the resolver is `private[layout]`. The generic context menu's own
  * `resolveContextMenu` was retired here (issue #819, slice 2) -- see `ContextMenuSurfaceCompositionSpec` instead.
  */
class PickerContentResolverSpec extends AnyFlatSpec with Matchers:
  ...
  // `resolveContextMenu` was retired (issue #819, slice 2): `ContextMenu` now paints entirely through
  // `ContextMenuSurfaceComposition`, which `SurfaceContentResolver.resolve` delegates to instead -- see
  // `ContextMenuSurfaceCompositionSpec` for coverage of its header, item, footer, and gap-row behavior.

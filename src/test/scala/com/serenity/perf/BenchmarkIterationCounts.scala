...
/** Per-family iteration counts for the benchmarks a false-positive audit of #1453/#1503's CI regression gate found
  ...
  * `lsp.framer.large_batch`, and `render.markdown.inline_lens`. Kept in one file, apart from the benchmarks
  * themselves, for the same reason [[BenchmarkFixtures]] is: the derivation is reviewed as a unit, separately from
  ...
  *
  *   - `damage.*` (30 -> 60, 2x): worst scenario (`single_char_short_line`) fell from 7.0% CV / 1.23x spread to
  *     2.85% CV / 1.10x spread. One count for the whole family: the worst scenario needed only a 2x bump, and
  *     applying it uniformly leaves every other scenario with margin rather than under-provisioned.
  *   - `reducer.*` (20 -> 60, 3x): the worst scenario shifted from `multi_cursor_move` (23.7% CV / 1.74x spread,
  *     traced to `BenchmarkRunner.calibrate`'s batch-size doubling: a noisy early sample can lock in half the batch
  *     size of a clean run, and that choice then holds for every sample in the run) to `reducer.normal_editing`
  *     (11.6% CV / 1.38x spread) -- `multi_cursor_move` itself dropped to 0.58% CV / 1.02x, but at ~0.014ms mean,
  *     `normal_editing` is small enough that raw sampling still shows real spread; 3x is not a complete fix by
  *     itself here, which is exactly why `check_perf_regression.py`'s absolute-ms floor exists as a second, required
  *     guard: `normal_editing`'s worst observed absolute delta was ~0.005ms, far under that floor.

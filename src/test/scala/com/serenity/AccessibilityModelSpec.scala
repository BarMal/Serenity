...
class AccessibilityModelSpec extends AnyFlatSpec with Matchers:
    ...
    cases.foreach {
      case (surfaceId, modal) =>
        ...
        val plan = ModalSurfaceComposition
          .forModal(modal, frame, SurfaceFrameLayout.minimumTargetRows(state.persisted.config.interfaceDensity))
  ...
  it should "expose the Open dialog's heading in the accessibility tree with the Heading role" in {
    ...
    val snapshot = AccessibilitySnapshot.from(state, viewport)
    val heading  = snapshot.nodes.find(_.id == s"surface:${surfaceId.value}/heading")

    heading.map(_.role) shouldBe Some(AccessibilityRole.Heading)
    heading.map(_.name) shouldBe Some("Open")
  }
  ...
  it should "align wrapped toolbar accessibility bounds with rendered row slots" in {
    ...
    val paneId    = PaneId(0)
    val buffer =
      Buffer.fromString(bufferId, "toolbar").copy(editing = EditingState(cursors = List(CursorPosition(0, 0))))
  ...
  it should "announce changed command and modal validation status once" in {
    ...
    val changed = AccessibilitySnapshot.from(
      baseline.copy(runtime = baseline.runtime.copy(uiSurfaces = baseline.runtime.uiSurfaces.map {
    ...
    val modalSnapshot = AccessibilitySnapshot.from(
      baseline.copy(runtime =
        baseline.runtime.copy(uiSurfaces =
          List(
            UiSurface(
              modalId,
              SurfaceContent.ModalWorkflow(

...
class CommandRunnerSettingsInputItemsSpec extends AnyFlatSpec with Matchers:
  ...
  "CommandRunnerSettingsInputItems" should "build config-backed settings input rows independently of runner state" in {
    val config = AppConfig.default
      .withInterfaceConfig(InterfaceConfig(elementGap = 0.5, outlineThicknessPx = 4))
    ...
    inputById(items, "ui-element-gap").currentValue shouldBe "0.5"
    ...
    inputById(items, "command-runner-item-gap-rows").currentValue shouldBe "0.25"
    inputById(items, "command-runner-item-gap-rows").parse("0.5") shouldBe
      Some(CommandIntent.SetCommandRunnerItemGapRows(0.5))
    inputById(items, "command-runner-cursor-gap-rows").currentValue shouldBe "0.75"

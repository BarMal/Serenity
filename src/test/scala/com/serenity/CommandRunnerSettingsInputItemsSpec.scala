...
class CommandRunnerSettingsInputItemsSpec extends AnyFlatSpec with Matchers:
  ...
  it should "build preset and binding parsers as reusable schema rows" in {
    ...
    inputById(items, "ui-preset-save-as-new").parse("Focus") shouldBe Some(CommandIntent.SaveUiPresetAsNew("Focus"))
    inputById(items, "ui-preset-overwrite").parse("Focus") shouldBe Some(CommandIntent.OverwriteUiPreset("Focus"))

...
class StateManagerCapabilitySpec extends AnyFlatSpec with Matchers:
  ...
  "StateManager" should "compose focused façade capabilities" in {
    ...
    // `peekManager` and `modalService` were retired in #1724; `bufferManager` followed in the same issue's next pass --
    // production drives their behavior through real events.
    val _: StateManager => (FileOpener, FileService, PanelManager) =
      sm => (sm.fileOpener, sm.fileService, sm.panelManager)
    val _: StateManager => (AnimationTicker, PaneManager) =
      sm => (sm.animationTicker, sm.paneManager)

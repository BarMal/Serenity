...
class EditorEventReducerOffsetSpec extends AnyFlatSpec with Matchers:
  ...
  "EditorCursorSupport.lineColumnToOffset" should "resolve line and column positions" in {
    val rope = Rope("alpha\nbeta\ngamma")

    EditorCursorSupport.lineColumnToOffset(rope, 0, 0) shouldBe 0
    EditorCursorSupport.lineColumnToOffset(rope, 0, 3) shouldBe 3
    EditorCursorSupport.lineColumnToOffset(rope, 1, 0) shouldBe 6
    EditorCursorSupport.lineColumnToOffset(rope, 1, 2) shouldBe 8
    EditorCursorSupport.lineColumnToOffset(rope, 2, 5) shouldBe rope.weight
  }

  it should "clamp negative and out-of-range positions" in {
    val rope = Rope("alpha\nbeta")

    EditorCursorSupport.lineColumnToOffset(rope, -1, -4) shouldBe 0
    EditorCursorSupport.lineColumnToOffset(rope, 0, 20) shouldBe 5
    EditorCursorSupport.lineColumnToOffset(rope, 20, 0) shouldBe rope.weight
  }
  ...
  it should "stop at the requested line without depending on later large content" in {
    ...
    EditorCursorSupport.lineColumnToOffset(rope, 0, 3) shouldBe 3
    EditorCursorSupport.lineColumnToOffset(rope, 1, 2) shouldBe 9
  }

...
class MockLspServer private (
  ...
  def push(notification: Json): IO[Unit] =
    logger.debug(s"[test] mock push ${notification.hcursor.downField("method").as[String].toOption.getOrElse("<response>")}").attempt.void >>
  ...
  private[lsp] def writerLoop: IO[Unit] =
    Stream.fromQueueNoneTerminated(outQueue)
      .evalMap(json =>
        logger.debug(s"[test] mock write ${json.hcursor.downField("method").as[String].toOption.getOrElse("<response>")}").attempt.void >>
          IO.blocking {
  ...
  private[lsp] def readerLoop: IO[Unit] =
    fs2.io.readInputStream(IO.pure(serverIn), 8192)
      .through(LspFramer.decode)
      .evalMap(json =>
        logger.debug(s"[test] mock read ${json.hcursor.downField("method").as[String].toOption.getOrElse("<response>")}").attempt.void >>
          received.offer(json) >>
          handleMessage(json)
      )

...
class LspProtocolSpec extends AnyFlatSpec with Matchers:
  ...
  it should "build references params carrying the position and includeDeclaration" in {
  ...
  it should "parse a list of reference locations, or None for a non-array result" in {
    ...
    parsed.map(_.map(_.uri)) shouldBe Some(List(DocumentUri("file:///foo/Bar.scala"), DocumentUri("file:///foo/Baz.scala")))

...
/** Shared fixtures for [[ContextualToolbarPlacementSpec]], [[ContextualToolbarDetailSpec]],
  * [[ContextualToolbarMouseSpec]], and [[ContextualToolbarDisplaySpec]] (state and interaction behaviour) and
  * [[ContextualToolbarLayoutSpec]] (cell-space geometry and hit-testing). Both drive the toolbar through a real
  * `StateManager` because the item list itself is state-derived; the split between the two specs is which half of

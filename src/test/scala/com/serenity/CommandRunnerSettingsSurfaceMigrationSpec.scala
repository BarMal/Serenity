...
class CommandRunnerSettingsSurfaceMigrationSpec extends AnyFlatSpec with Matchers:
  ...
  "enterSelectedGroup" should "push a single-page stack, with no ancestors, for a top-level group" in {
    val entered = opened.withSelectedItem("settings-editor").enterSelectedGroup

    entered.activeSettingsSurface shouldBe Some(SettingsSurfaceState(SettingsPage.Group("settings-editor")))
  }
  ...
  it should "build ancestor pages matching the preferred ancestor path when jumping to a search leaf" in {
    ...
    entered.activeSettingsSurface.map(_.ancestors.map(_.groupId)) shouldBe Some(List("settings-editor"))
  }
  ...
  it should "close the whole stack when there is no parent to reveal" in {
    val level1 = opened.withSelectedItem("settings-look").enterSelectedGroup
  ...
  "moveSubmenuSelection" should "advance the current page's index" in {
    ...
    moved.activeSettingsSurface shouldBe Some(SettingsSurfaceState(SettingsPage.Group("settings-look", 1)))
  }
  ...
  it should "drop an Editing page back to Group, mirroring the old model always exiting edit mode" in {
    ...
    moved.activeSettingsSurface.map(_.current) shouldBe Some(SettingsPage.Group("settings-look-advanced", 1))
  }
      ...
      case Some(surface) =>
        editing.withDrilledSettingsSurface(
          surface.copy(current =
            SettingsPage.Editing(groupId = "settings-look-advanced", itemId = "no-such-item", draftText = "")
          )
        )
  ...
  "withSelectedFocusedSubmenuIndex" should "set the current page's index directly, mirroring moveSubmenuSelection" in {
    ...
    moved.activeSettingsSurface shouldBe Some(SettingsSurfaceState(SettingsPage.Group("settings-look", 3)))
  }
  ...
  it should "drop an Editing page back to Group" in {
    ...
    moved.activeSettingsSurface.map(_.current) shouldBe Some(SettingsPage.Group("settings-look-advanced", 2))
  }
  ...
  "focusedSubmenuItems" should "read through activeSettingsSurface, unfiltered" in {
    ...
    level1.focusedSubmenuItems.map(_.id) shouldBe level1.submenuItems("settings-look").map(_.id)
  }
  ...
  "settingsSurfaceItems" should "read through activeSettingsSurface once inside a group" in {
    ...
    level1.settingsSurfaceItems.map(_.id) shouldBe level1.submenuItems("settings-look").map(_.id)
  }
  ...
  "cancelSubmenuEditingText" should "cancel an in-progress edit, reverting to a Group page" in {
    ...
    cancelled.activeSettingsSurface.map(_.current) shouldBe Some(SettingsPage.Group("settings-look-advanced", 0))
  }
  ...
  "clearSubmenuEditingAndRecording" should "clear all editing/recording sub-state, reverting to a Group page" in {
    ...
    cleared.activeSettingsSurface.map(_.current) shouldBe Some(SettingsPage.Group("settings-look-advanced", 0))
  }

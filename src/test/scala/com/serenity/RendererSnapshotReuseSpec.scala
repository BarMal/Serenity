...
class RendererSnapshotReuseSpec extends AnyFlatSpec with Matchers:
  ...
  it should "query a visible comment without scanning unrelated indexed ranges" in {
    ...
    val result = state
      .annotationIndex(bufferId)
      .getOrElse(fail("expected an index for a buffer that exists"))
      .commentsByLine(Set(100000))

    result.values.flatten.toList shouldBe List(visible)
  }

  // #1456: annotationIndexByBuffer/markdownFenceIndexByBuffer were `lazy val`s that rebuilt a whole-workspace wrapper
  // map -- one entry per buffer -- on first touch, just to reach a single buffer's index, even though the expensive
  // per-buffer index computation itself was already deferred behind a thunk. Since a fresh `AppState` snapshot is
  // produced on essentially every edit (each with its own unforced `lazy val`), that O(buffers) map build repeats on
  // every first lookup against a new snapshot. This logs the real wall-clock cost of that first lookup against a
  // fresh snapshot of a workspace with a large buffer count, in the same spirit as `WordStatisticsPerformanceSpec`.

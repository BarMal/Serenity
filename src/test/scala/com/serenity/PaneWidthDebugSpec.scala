...
class PaneWidthDebugSpec extends AnyFlatSpec with Matchers:
  ...
  it should "debug the actual behavior when creating many small panes" in new DebugFixture:
    ...
    paneLayouts1.foreach((paneId, rect) => println(s"  PaneId(${paneId.value}) -> x=${rect.x}, width=${rect.width}"))

    println(s"\nWith 50 char minimum: ${paneLayouts50.size} layouts")
    paneLayouts50.foreach((paneId, rect) => println(s"  PaneId(${paneId.value}) -> x=${rect.x}, width=${rect.width}"))
    ...
    val visiblePanes1 =
      paneLayouts1Actual.values.count(rect => rect.x >= editorRect.x && rect.x < (editorRect.x + editorRect.width))

    println(s"\nWith 1 char minimum: $visiblePanes1 visible panes out of ${finalState.layout.editorPanes.size}")
    println(
      s"Expected problem: All 6 panes visible with ~${editorArea / 6} chars each = ${editorArea / 6} chars per pane"
    )

    // This should demonstrate the issue you're seeing
    if visiblePanes1 == finalState.layout.editorPanes.size then
      ...
      if avgWidth < 20 then println("CONFIRMED: Panes are too narrow (< 20 chars)")
